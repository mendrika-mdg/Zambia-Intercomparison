...
def load_ZCAST_nowcast(year, month, day, hour, minute, lead_time):
    base_dir = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/Intercomparison/zcast/ensemble_nowcasts/t{lead_time}/{year}{month}/{hour}"
    file = f"{base_dir}/zcast_mean_{year}{month}{day}_{hour}{minute}.npy"
    if not os.path.exists(file):
...
def save_city_nowcasts_by_lead(yx_locations, nowcast_origin, lead_time):
    base_save_dir = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/Intercomparison/time_series/zcast_ensemble/t{lead_time}"
...
# ---------------------------------------------------------------------
zcast_root = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/Intercomparison/zcast/ensemble_nowcasts/t{lead_time}"
pattern = re.compile(r"zcast_mean_(\d{4})(\d{2})(\d{2})_(\d{2})(\d{2})\.npy$")
...
for entry in tqdm(nowcast_files, desc=f"Processing lead time {lead_time}"):
    ...
    try:
        ...
        for name, info in locations.items():
            ...
            zcast_val = float(np.mean(extract_box(zcast, y_zcast, x_zcast)))
            gt_val     = int(np.max(extract_box(ground_truth, y_gt, x_gt)))

            yx_locations[name] = (y_zcast, x_zcast, round(zcast_val, 4), gt_val)
