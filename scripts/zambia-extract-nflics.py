...
def load_NFLICS_nowcast(year, month, day, hour, minute, lead_time):
    base_dir = f"/gws/ssde/j25b/swift/nflics_nowcasts/{year}/{month}/{day}/{hour}{minute}"
    file = f"{base_dir}/Nowcast_{year}{month}{day}{hour}{minute}_000_sadc.nc"
    if not os.path.exists(file):
        raise FileNotFoundError(f"Missing NFLICS file: {file}")
    data = Dataset(file, mode='r')["Probability"][int(lead_time), :, :]
    return data
...
def save_city_nowcasts_by_lead(yx_locations, nowcast_origin, lead_time):
    ...
    for city, (_, _, nflics_val, gt_val) in yx_locations.items():
...
# ---------------------------------------------------------------------
# Automatically detect all available NFLICS nowcasts
# ---------------------------------------------------------------------

nflics_root = "/gws/ssde/j25b/swift/nflics_nowcasts"
pattern = re.compile(r"Nowcast_(\d{4})(\d{2})(\d{2})(\d{2})(\d{2})_000_sadc\.nc$")

nowcast_files = []
for root, _, files in os.walk(nflics_root):
    for f in files:
        m = pattern.match(f)
        if m:
...
print(f"Detected {len(nowcast_files)} NFLICS nowcast files.")
...
for entry in tqdm(nowcast_files, desc=f"Processing lead time {lead_time}"):
    ...
    try:
        ...
        for name, info in locations.items():
            y_nflics = info["y_nflics"]
            x_nflics = info["x_nflics"]
            y_gt = info["y_gt"]
            x_gt = info["x_gt"]
            ...
            yx_locations[name] = (y_nflics, x_nflics, round(nflics_val, 4), gt_val)
