...
class ShardDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        lag_col = 9
        x[..., lag_col] = 2 * (x[..., lag_col] / 120.0) - 1.0  # scale 0–120 → −1…1
        return x, g, y

# ---------------------------------------------------------------------
# Decoder (smaller, lighter)
# ---------------------------------------------------------------------
class SmoothDecoder(nn.Module):
    def __init__(self, embed_dim: int, out_hw=(350, 370)):
        super().__init__()
        self.out_hw = out_hw
        ch = [embed_dim, 256, 128, 64, 32, 16]
        layers = []
...
# ---------------------------------------------------------------------
# Core2MapModel with 32×32 latent map
# ---------------------------------------------------------------------
class Core2MapModel(pl.LightningModule):
    def __init__(self, embed_dim=128, num_heads=4, num_layers=4, lr=1e-4, out_hw=(350, 370)):
        ...
        # Latent projection to 32×32
        self.latent_hw = (32, 32)
        self.map_proj = nn.Linear(embed_dim, embed_dim * self.latent_hw[0] * self.latent_hw[1])
    ...
    def forward(self, x, g):
        ...
        x = self.map_proj(combined).view(b, -1, *self.latent_hw)
...
def main():
    ...
    BATCH_SIZE = 32
    CHECKPOINT_DIR = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/Intercomparison/checkpoints/t{LEAD_TIME}_32x32"
    ...
    logger = WandbLogger(project="zambia-cast", name=f"t{LEAD_TIME}-32x32-lag-scaled")
