...
class SpatiallyEnhancedLoss(nn.Module):
    def __init__(self, window_size: int = 9, pos_weight: float = 25.0, alpha: float = 0.3):
        """
...
class Core2MapModel(pl.LightningModule):
    def __init__(self, embed_dim=128, num_heads=4, num_layers=4,
        ...
        self.criterion = SpatiallyEnhancedLoss(window_size=16, pos_weight=pos_weight, alpha=alpha)
...
def main():
    ...
    ckpt_dir = f"/gws/nopw/j04/wiser_ewsa/mrakotomanga/Intercomparison/checkpoints/ensemble_hybrid/t{lead_time}/seed{seed}"
    ...
    model = Core2MapModel(embed_dim=128, num_heads=4, num_layers=4,
                          dropout_p=0.2, pos_weight=25.0, alpha=0.3)
    logger = WandbLogger(project="zambia-ensemble-hybrid", name=f"t{lead_time}-seed{seed}")
    ...
    trainer = pl.Trainer(
        ...
        callbacks=[
            ...
            EarlyStopping(monitor="val_fss_9", mode="max",
