...
def compute_fss(preds: torch.Tensor, targets: torch.Tensor, window: int = 9) -> torch.Tensor:
    """Compute Fractions Skill Score (FSS) on [0,1] tensors."""
...
class Core2MapModel(pl.LightningModule):
    def __init__(self, embed_dim=128, num_heads=4, num_layers=4, lr=1e-4, out_hw=(350, 370)):
        ...
        # Transformer encoder for core-level features
        ...
        # Map projection and decoder
        self.map_proj = nn.Linear(embed_dim, embed_dim * 16 * 16)
        self.decoder = SimpleDecoder(embed_dim, out_hw=out_hw)

        # FSS loss and metrics
    ...
    def forward(self, x, g):
        ...
        x = self.map_proj(combined).view(b, -1, 16, 16)
    ...
    def validation_step(self, batch, _):
        ...
        for w in [3, 5, 9]:
            ...
            self.log(f"val_fss_{w}", fss_val, prog_bar=False, sync_dist=True)
...
def main():
    ...
    CHECKPOINT_DIR = f"/home/users/mendrika/Zambia-Intercomparison/models/checkpoints/t{LEAD_TIME}"
    ...
    train_loader = DataLoader(
        ...
        num_workers=2, persistent_workers=False, pin_memory=True, prefetch_factor=2
    )
    ...
    logger = WandbLogger(project="zambia-cast", name=f"t{LEAD_TIME}-fss")
